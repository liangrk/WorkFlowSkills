...
<!--
  ...
  1. **所有 tasks.json 写入必须使用原子写入脚本:**
     bash "$SHARED_BIN/android-json-atomic" <filepath>
     读取 stdin 中的 JSON，验证后通过 temp+rename 原子写入。

  2. **所有读取-修改-写入序列必须加文件锁:**
     bash "$SHARED_BIN/android-file-lock" <lockfile> <command> [args...]
...
> 所有 `android-file-lock`、`android-json-atomic` 等脚本调用必须使用
> `bash "$SHARED_BIN/android-file-lock"` 形式，**禁止使用相对路径**。
> 在 plan worktree 中执行时，相对路径 `.claude/skills/...` 无法定位到主 worktree 的脚本。
...
```json
{
  ...
  "plans": {
    "plan-<时间戳>": {
      ...
      "source": "autoplan | investigate | fix | manual | null",
      "approved_at": "ISO-8601 | null",
      "review_summary": {
        "mechanical_decisions": 0,
        "taste_decisions": 0,
        "user_challenges": 0
      } | null,
      "prd": {
        "functional_requirements": ["FR-1: ...", "FR-2: ..."],
        "acceptance_criteria": ["AC-1: [FR-1] ...", "AC-2: [FR-2] ..."],
        "exclusions": ["..."]
      },
      "worktree": {
...
| `prd` | plan 级别 | PRD 解析结果，包含 `functional_requirements`、`acceptance_criteria`、`exclusions`。若 plan 无 PRD section 则为 `null` |
| `prd_notes` | task 级别 | PRD 验证备注，记录未实现或有问题的 AC 编号。若无问题则为 `null` |
...
从 plan 文件中提取 PRD section:

```bash
PLAN_FILE="<plan-file-path>"
# Extract PRD section (between ## PRD and the next ## heading)
...
将解析结果存储在 tasks.json 的 plan 级别字段:
...
若 plan 文件中无 PRD section，记录 `prd: null`，后续验证自动跳过。

### 步骤 4.6: 读取 Plan Status (审批凭证)

检查是否存在 autoplan 生成的 plan-status.json (write-once 审批凭证):
...
**若存在 plan-status.json:**

1. 提取 plan_id 并赋值:
```bash
STATUS_FILE="${PLAN_FILE%.md}-status.json"
IMPORTED_PLAN_ID=$(python3 -c "import json; print(json.load(open('$STATUS_FILE'))['plan_id'])" 2>/dev/null || "")
```

2. 读取其中的元数据:
   - `plan_id`: 优先使用此 ID (与 autoplan 对齐)
   - `source`: 记录到 tasks.json 的 plan 级别字段
   - `review_summary`: 记录到 tasks.json 的 plan 级别字段
   - `approved_at`: 记录到 tasks.json 的 plan 级别字段

2. 将元数据合并到 tasks.json 的 plan 对象中:
```json
{
  "source": "autoplan",
...
3. **不修改 plan-status.json** (write-once 原则)。

**source 与 plan_source.type 的区别:**

这两个字段追踪不同的维度，不应混淆:

| 字段 | 含义 | 取值 | 来源 |
|------|------|------|------|
| `source` | 哪个 skill 产出了这个 plan | `autoplan` / `investigate` / `fix` / `manual` | plan-status.json |
| `plan_source.type` | plan 的格式/来源类型 | `memory` / `markdown` / `gstack-design` / `superpowers` / `custom` | runner 自行判断 |

- `source` 为 `null` 时表示该 plan 未经过 autoplan 审批流程 (手动创建等)
- 两个字段独立存在，不做互相映射

**若不存在 plan-status.json:**

正常继续，使用 runner 自己生成的 plan_id。这种情况出现在:
- plan 文件不是由 autoplan 生成的 (手动创建、investigate/fix 产出)
- autoplan 的早期版本 (未生成 status 文件)

### 步骤 5: 创建 Worktree 并写入状态

```bash
# 生成 plan ID 和 slug (优先使用 plan-status.json 中的 plan_id)
if [ -n "$IMPORTED_PLAN_ID" ]; then
...
当状态展示中存在 `⊘` (blocked) 或 `✗` (failed) 状态的任务时:

1. 在状态展示后追加恢复建议:
   ```
   === 被阻塞/失败的任务 ===
   ⊘ 任务 4: Native 层构建验证 — 阻塞原因: bazel 未安装
   ✗ 任务 2: 用户认证逻辑 — 失败原因: test_failure (3 轮修复后仍有 2 个测试失败)
   ```
2. 使用 AskUserQuestion 询问:
   > 检测到 1 个被阻塞任务和 1 个失败任务，是否处理?
   > - A) 重试所有阻塞/失败任务 (重置为 pending)
   > - B) 仅重试阻塞任务 (需先手动安装缺失工具)
   > - C) 跳过，继续正常流程

3. 选择 A 或 B 时:
   - **检查 worktree 脏状态:** 在重置前，检查目标任务的 worktree 是否有未提交变更
     ```bash
     git -C "<worktree_path>" status --porcelain
     ```
     - 有未提交变更 → 询问用户: "放弃变更重试" 或 "保留变更但可能冲突"
   - 将目标任务的 `status` 重置为 `"pending"`
   - 清除 `failure_reason` 字段
   - 重置 `timestamps.started` 为 `null`
   - 重置 TDD 状态 (如存在): `tdd.executed` → `false`, `tdd.coverage_percent` → `null`, `tdd.report_path` → `null`
   - 如果并行模式中有下游任务因依赖被 `blocked`，同步清除其 `blocked` 状态
   - 然后进入 Phase 2 执行
...
> **参考:** 详见 `REFERENCE.md` — 工具安装指南（仅缺失工具时需要）。

  subagent prompt 的完整内容见 REFERENCE.md「缺失工具」章节。
```
...
工具可用不代表版本正确。在确认工具存在后，检测版本是否满足项目要求:
...
# JDK: 解析 build.gradle / gradle.properties
if echo "$BUILD_SYSTEM" | grep -q "gradle"; then
  REQUIRED_JAVA=""
...
if [ -n "$VERSION_MISMATCH" ]; then
  echo "VERSION_MISMATCH: $VERSION_MISMATCH"
else
  echo "VERSION_CHECK: passed"
fi
```

**版本不匹配的处理:** 与工具缺失相同 — 派发 subagent 尝试安装正确版本，
失败则标记为 `"blocked"` + `failure_reason: "version_mismatch"`。
...
| 3 轮修复后仍有失败 | 标记任务为 `"failed"` + `failure_reason: "test_failure"`，中止任务，不提交 |
...
若 tasks.json 中当前 plan 的 `prd` 不为 `null`:

1. 根据当前任务的 `title` 和 `steps`，匹配相关的验收标准 (AC-N)
2. 对每条匹配的 AC，验证实现是否满足:
   - **代码层面:** 相关功能代码是否存在且正确
   - **测试层面:** 是否有测试覆盖该 AC
3. 输出:
   ```
   [PRD 验证] Task "<title>"
   - AC-1 [FR-1]: ✓ 实现完整 (test: LoginViewModelTest#loginSuccess)
   - AC-3 [FR-2]: ⚠ 部分实现 (错误信息不够具体，建议区分"用户不存在"和"密码错误")
   - AC-5 [FR-4]: ✗ 未实现 (记住我功能尚未编码)
   ```
4. 若有 ✗ 项:
   - 标记为 warning (不阻塞任务完成，但记录到任务备注)
   - 在 tasks.json 中更新任务状态:
     ```json
     {"status": "completed", "prd_notes": "AC-5 未实现"}
     ```
5. 若全部 ✓: 输出 "[PRD 验证] 全部通过"

**若 `prd` 为 `null`:** 跳过 PRD 验证，不输出任何内容。

**重要:** PRD 验证不阻塞任务完成。即使有 ✗ 项，任务仍可提交。
它仅作为信息性检查，帮助追踪需求覆盖度。
...
**同步写入 PRD 验证结果 (如果适用):**
- 如果 PRD 验证发现有 ✗ 项: 确保 `prd_notes` 已写入 (如 `"AC-5 未实现"`)
- 如果 PRD 验证全部通过: `prd_notes` 保持 `null`
- 如果无 PRD (`prd` 为 `null`): `prd_notes` 保持 `null`
...
print(json.dumps(data, indent=2, ensure_ascii=False))
" | bash "$SHARED_BIN/android-file-lock" "$TASKS_JSON.lock" \
    bash "$SHARED_BIN/android-json-atomic" "$TASKS_JSON"
```
...
5. 执行 PRD 验证 (若 plan 的 prd 不为 null):
   ...
   - 输出格式: `[PRD 验证] Task "<title>"` + 每条 AC 的状态 (✓/⚠/✗)
   - 若有 ✗ 项: 记录到 prd_notes (不阻塞任务完成)
6. 验证通过后提交:
   git add -A
   git commit -m "feat(<plan-slug>): complete task <task-num> - <task-title>"
7. 更新 tasks.json (必须使用 `bash "$SHARED_BIN/android-json-atomic"` + `bash "$SHARED_BIN/android-file-lock"`):
   - status → completed
   - commit → commit hash
...
8. 如果验证失败，更新 tasks.json:
   - status → failed
   - failure_reason → "test_failure" (测试失败) 或具体错误描述
   - 在 commit 字段中记录错误摘要
...
> **参考:** 详见 `REFERENCE.md` — Plan 完成输出格式（仅 Plan 完成时 Read）。

---

> **参考:** 详见 `REFERENCE.md` — 多 Worktree 管理与冲突预防（高级功能，按需 Read）。

---

> **参考:** 详见 `REFERENCE.md` — 恢复机制与持久化规则（按需查阅）。
...
1. **构建顺序依赖** — 如果发现某些任务必须按特定顺序执行（如 Gradle 插件配置必须在模块创建之前），记录:
   ```bash
   bash "$SHARED_BIN/android-learnings-log" '{"skill":"worktree-runner","type":"technique","key":"<依赖描述>","insight":"<顺序要求和原因>","confidence":8,"source":"observed","files":[]}'
   ```

2. **环境配置问题** — 如 Gradle daemon 内存不足、worktree 权限问题，记录:
   ```bash
   bash "$SHARED_BIN/android-learnings-log" '{"skill":"worktree-runner","type":"technique","key":"<环境问题>","insight":"<问题描述和解决方案>","confidence":7,"source":"observed","files":[]}'
   ```
...
| 测试验证失败 | 不允许跳过，最多 3 轮自动修复，仍失败则标记 `failed` + `failure_reason: "test_failure"` 并中止任务 |
...
> **参考:** 详见 `REFERENCE.md` — worktree-info.md 文件格式。

---

> **参考:** 详见 `REFERENCE.md` — 文件结构与 Skill 集成。
