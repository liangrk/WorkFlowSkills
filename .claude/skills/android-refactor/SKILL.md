...
```bash
LEARNINGS=$(bash .claude/skills/android-shared/bin/android-learnings-search --type pattern --limit 5 2>/dev/null || true)
if [ -n "$LEARNINGS" ]; then
  echo "=== 相关学习记录 ==="
  echo "$LEARNINGS"
fi
...
# 统计测试文件数
find app/src/test -name "*Test.kt" -o -name "*Spec.kt" 2>/dev/null | wc -l
...
```bash
bash .claude/skills/android-shared/bin/android-learnings-log '{"skill":"android-refactor","type":"pattern","key":"<refactoring-pattern>","insight":"<insight>","confidence":8,"source":"observed","files":["<files>"]}'
```
